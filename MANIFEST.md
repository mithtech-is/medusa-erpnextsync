...
- `plugin/` — the 3 changed generic-plugin files (whole copies): `registry.ts`,
  `index.ts`, `admin/routes/erpnext/page.tsx`.
...
- `frappe/` — medusync handlers (`inventory.py`, `sales_financials.py`,
  `reverse.py`, patched `mapped.py`), patched `hooks.py`, and the
  setup/seed/patcher scripts (Custom Fields, Property Setter, mappings).
...
## Status
All features verified live (see each `*_RESULTS.md`). **Not committed to the
real repos, not pushed.** Returns/Refunds and Pricing/B2B remain (see audit).
